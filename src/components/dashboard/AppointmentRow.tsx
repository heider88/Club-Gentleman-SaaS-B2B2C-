            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
                {!isAdmin && appt.customer_phone && appt.customer_phone !== "N/A" && (
                    <div className="flex gap-1 mr-1">
                        <a href={`https://wa.me/${((p) => { const c = p.replace(/\\D/g, ''); return c.length === 10 ? '57' + c : c; })(appt.customer_phone)}`} target="_blank" rel="noreferrer" className="w-9 h-9 flex items-center justify-center border border-dash-border bg-dash-panel text-dash-text-soft hover:text-dash-text hover:bg-dash-panel-alt transition-colors active:scale-95" title="WhatsApp">
