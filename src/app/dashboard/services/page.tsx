...
export default function ServicesPage() {
    ...
    if (loading) {
        return (
            <div className="p-8 text-center animate-pulse flex flex-col items-center justify-center space-y-4">
                <Scissors className="w-10 h-10 text-dash-text/20 animate-spin" />
                <span className="text-dash-text/50 font-medium">Afilando tijeras... (Cargando catálogo)</span>
            </div>
        )
    }
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
            ...
            {isEditing ? (
                <div className="bg-dash-panel/30 backdrop-blur-md border border-white/10 rounded-3xl p-6 sm:p-10 animate-in fade-in zoom-in-95 shadow-2xl relative overflow-hidden">
                    {/* Background glow */}
                    <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full blur-[100px] pointer-events-none" />

                    <h2 className="text-2xl font-bold border-b border-white/10 pb-4 mb-8 text-dash-text flex items-center gap-3">
                    ...
                    <form onSubmit={handleSave} className="space-y-8 relative z-10">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-dash-text/50 uppercase tracking-wider pl-1">Nombre del Servicio *</label>
                                <input
                                    type="text"
                                    name="name"
                                    required
                                    ...
                                    className="w-full p-4 rounded-xl bg-dash-panel/50 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all text-dash-text font-medium"
                                />
                            </div>

                            <div className="space-y-2">
                                <label className="text-xs font-bold text-dash-text/50 uppercase tracking-wider pl-1">Precio Cobrado *</label>
                                <div className="relative">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-dash-text/50 font-bold">$</span>
                                    <input
                                        type="number"
                                        name="price"
                                        required
                                        ...
                                        onChange={handleChange}
                                        placeholder="0"
                                        className="w-full p-4 pl-8 rounded-xl bg-dash-panel/50 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all text-dash-text font-bold"
                                    />
                                </div>
                            </div>

                            <div className="space-y-2 sm:col-span-2 md:col-span-1">
                                <label className="text-xs font-bold text-dash-text/50 uppercase tracking-wider pl-1">Tiempo Asignado en Agenda (Minutos) *</label>
                                <div className="relative">
                                    <input
                                        type="number"
                                        ...
                                        onChange={handleChange}
                                        placeholder="Ej. 40"
                                        className="w-full p-4 pr-12 rounded-xl bg-dash-panel/50 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all text-dash-text font-medium"
                                    />
                                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-dash-text/50 font-bold text-sm">min</span>
                                </div>
                            </div>
                        </div>

                        <div className="space-y-2">
                            <label className="text-xs font-bold text-dash-text/50 uppercase tracking-wider pl-1">Descripción Breve (Opcional)</label>
                            <textarea
                                name="description"
                                ...
                                className="w-full p-4 rounded-xl bg-dash-panel/50 border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all text-dash-text resize-y"
                            />
                        </div>
                        ...
                        <div className="flex justify-end gap-4 pt-6 border-t border-white/10 mt-8">
                            <button
                                ...
                                disabled={saving}
                                className="flex items-center gap-2 px-6 py-3 rounded-xl text-dash-text/70 hover:bg-dash-text/5 hover:text-dash-text font-bold transition-all border border-transparent hover:border-white/10"
                            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {services.length === 0 ? (
                        <div className="col-span-full flex flex-col items-center justify-center py-20 border border-dashed border-white/20 rounded-3xl bg-dash-panel/20 backdrop-blur-sm group">
                            <Ban className="w-16 h-16 text-dash-text/10 group-hover:text-primary/50 transition-colors duration-500 mb-4" />
                            <h3 className="text-xl font-bold text-dash-text/80 mb-2">Vitrina Vacía</h3>
                            ...
                            <div key={service.id} className="bg-card border border-white/10 rounded-3xl p-6 hover:border-primary/40 hover:shadow-[0_8px_30px_rgba(var(--color-primary),0.1)] transition-all flex flex-col justify-between group h-full">
                                <div>
                                    <div className="flex justify-between items-start mb-4">
                                        <h3 className="text-lg font-bold text-dash-text group-hover:text-primary transition-colors leading-tight">
                                            {service.name}
                                        </h3>
                                    ...
                                    <p className="text-dash-text/50 text-sm mb-6 line-clamp-3 leading-relaxed">
                                ...
                                <div className="flex items-center justify-between border-t border-white/5 pt-5 mt-auto">
                                    <span className="text-xs font-bold text-dash-text/40 flex items-center gap-1.5 bg-dash-panel/50 px-3 py-1.5 rounded-lg border border-white/5">
                                    ...
                                    <div className="flex gap-2 opacity-100 sm:opacity-50 sm:group-hover:opacity-100 transition-opacity">
                                        <button
                                            ...
                                            className="p-2 rounded-xl bg-dash-text/5 hover:bg-dash-text/10 text-dash-text transition-all hover:text-primary"
                                            title="Editar"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="p-2 rounded-xl bg-dash-text/5 hover:bg-destructive/20 text-dash-text hover:text-destructive transition-all"
                                            title="Eliminar"
                                        >
