    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 max-w-6xl mx-auto">
            <header className="flex flex-col gap-4 border-b border-white/5 pb-6">
                <Link href="/dashboard/admin" className="text-white/50 hover:text-white flex items-center gap-2 text-sm w-fit transition-colors bg-white/5 px-4 py-2 rounded-xl">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-2 space-y-8">
                    ...
                    <section className="bg-card/90 backdrop-blur-xl border border-border rounded-3xl p-6 shadow-xl">
                        ...
                        <p className="text-white/50 text-sm mb-6">Administra los cortes y precios específicos que ofrece este barbero o impórtalos del catálogo global.</p>
                    ...
                    <section className="bg-card/90 backdrop-blur-xl border border-border rounded-3xl p-6 shadow-xl">
                        ...
                        <div className="space-y-3 max-h-[400px] overflow-y-auto scrollbar-hide pr-2">
                            {appointments && appointments.length > 0 ? appointments.map((appt: any) => (
                                <div key={appt.id} className="p-4 bg-black/40 border border-white/10 rounded-xl">
                                    <div className="flex justify-between items-start mb-2">
                                        <p className="text-white font-bold text-sm">{new Date(appt.start_time).toLocaleString('es-CO', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</p>
                                    ...
                                    <p className="text-white/70 text-sm">{appt.customer_name}</p>
                                    <p className="text-white/40 text-xs mt-1">{appt.services?.name || 'Servicio Externo'}</p>
                                </div>
                            )) : <p className="text-white/40 text-sm text-center py-4 bg-white/5 rounded-xl border border-dashed border-white/10">El barbero no tiene citas futuras.</p>}
                        </div>
                    </section>
                </div>
            </div>
